...
const UnmatchedPage: React.FC = () => {
    ...
    if (loading) return <div style={{ padding: '100px', textAlign: 'center', color: 'var(--primary)' }}>{t('pages.unmatched.loading')}</div>;

    return (
        <div style={{ maxWidth: '1400px', padding: '24px', margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '32px' }}>
                <div>
                    <Typography variant="h1" style={{ fontSize: '32px', marginBottom: '8px', fontWeight: 800 }}>{t('pages.unmatched.title')}</Typography>
                    <Typography variant="body" style={{ color: 'var(--text-secondary)' }}>
                        {t('pages.unmatched.description')}
                    </Typography>
                </div>
                <div style={{ position: 'relative', width: '300px' }}>
                    ...
                    <input
                        type="text"
                        placeholder={t('pages.unmatched.searchPlaceholder')}
                        value={searchQuery}
            ...
            <Card style={{ padding: 0, overflow: 'hidden', marginBottom: '24px' }}>
                <Table headers={[t('pages.unmatched.sections.userInfo'), t('pages.unmatched.sections.lastAttempt'), t('pages.unmatched.sections.steps'), t('pages.unmatched.sections.criteria'), t('pages.unmatched.sections.actions')]}>
