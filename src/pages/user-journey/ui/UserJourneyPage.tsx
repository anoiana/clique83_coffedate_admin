...
const UserJourneyPage: React.FC = () => {
                ...
                <div style={{ 
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                        ...
                        <div>
                            ...
                            <Typography variant="caption" style={{ color: 'var(--text-secondary)' }}>{t('pages.userJourney.funnelTitle')}</Typography>
                        </div>
                    </div>

                    <div style={{ textAlign: 'right' }}>
                        <Typography variant="h1" style={{ margin: 0, fontSize: '28px', fontWeight: 800 }}>{stage.value}</Typography>
                        <Typography variant="caption" style={{ color: 'var(--text-secondary)' }}>{t('pages.userJourney.totalUsers')}</Typography>
                    </div>
                </div>
                ...
                {nextValue !== undefined && (
                    <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                        ...
                        <div style={{ 
                            ...
                            <TrendingDown size={12} /> {dropoff}% {t('pages.userJourney.dropoff')}
                        </div>
    ...
    if (loading) return <div style={{ padding: '100px', textAlign: 'center' }}>{t('pages.userJourney.loading')}</div>;

    return (
        <div style={{ maxWidth: '800px', padding: '24px', margin: '0 auto' }}>
            <div style={{ marginBottom: '40px', textAlign: 'center' }}>
                <Typography variant="h1" style={{ fontSize: '32px', marginBottom: '12px', fontWeight: 800 }}>{t('pages.userJourney.title')}</Typography>
                <Typography variant="body" style={{ color: 'var(--text-secondary)' }}>
                    {t('pages.userJourney.description')}
                </Typography>
            </div>
            ...
            <Card style={{ marginTop: '40px', background: 'rgba(255,215,0,0.02)', border: '1px dashed var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    <div>
                        <Typography variant="body" style={{ fontWeight: 700 }}>{t('pages.userJourney.optimization')}</Typography>
                        <Typography variant="caption" style={{ color: 'var(--text-secondary)', display: 'block' }}>
                            {t('pages.userJourney.conversionRate', { rate: stages.length > 0 ? ((stages[stages.length-1].value / stages[0].value) * 100).toFixed(2) : 0 })}
                        </Typography>
                    </div>
                </div>
            </Card>
